...
func newDropScreen(display display) *dropScreen {
	return &dropScreen{
		display: display,
		menu:    newDropPanel(display),
	}
}

// Render the drop screen.
func (p *dropScreen) Render(g *game.Game) {
	p.menu.Render(g)
}

// Handle an event generated by the game after the last command.
func (p *dropScreen) Handle(ev game.Event) {
}

// Get the next command from the player to be sent to the game instance.
func (inv *dropScreen) NextCommand() game.Command {
	for {
		tboxev := inv.display.PollEvent()
		if tboxev.Type != termbox.EventKey {
			continue
...
// Listens to nothing.
func (m *dropPanel) Handle(e game.Event) {
}

// Render the menu.
func (m *dropPanel) Render(g *game.Game) {
	inv := g.Player.Packer.Inventory()
	renderInventory(m.display, "Drop what?", inv)
}
